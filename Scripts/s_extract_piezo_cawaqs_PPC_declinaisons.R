...
# Scenario names
scNames <- c("R060","R070","R080","R090","R100","R115")

scColor <- c("#ffc265", "#e88e05","#FF5733","#C70039","#900C3F","#581845")
...
# Pdf output filename
outputPdfFile <- paste('output_stationMETRO.pdf',sep='')
...
for (sc in (1:length(scNames)))
{
   # Simulation name
   simname1 <- paste('SC_1910_',scNames[sc],"_STOP",sep="")
   simname2 <- paste('SC_1910_',scNames[sc],"_POMP",sep="")
   simname3 <- paste('SC_1910_',scNames[sc],"_TOUS",sep="")
   simname4 <- paste('SC_1910_',scNames[sc],"_NONE",sep="")
   ...
   for (i in (1:nbPiezo))
   {
      ...
      # Setting title
      figTitle <- paste("Scénario : ",scNames[sc]," / Localisation : ",properties[i,5]," : ",properties[i,4],sep="")
      ...
      pl =  ggplot(dataFrame1) +
         geom_line(aes(x = dataFrame1[,1], y = dataFrame1[,2*i+1], color = '(1) Arrêt ZINI'), size = 1.1, alpha = 1.0)  +
         geom_line(aes(x = dataFrame2[,1], y = dataFrame2[,2*i+1], color = '(2) Pompages persistants'), size = 0.7, alpha = 0.5)  +
         geom_line(aes(x = dataFrame3[,1], y = dataFrame3[,2*i+1], color = '(3) Arrêt global des pompages'), size = 0.7, alpha = 0.5)  +
         #geom_line(aes(x = dataFrame4[,1], y = dataFrame4[,2*i+1], color = '(4) Sans pompages'), size = 0.4, alpha = 0.5)  +
         
#         geom_line(aes(x = dataFrameMNT[,1], y = 21.47, color = '(4) Alti min.'), size = 0.4, alpha = 0.5, linetype = "dashed")  +
#         geom_line(aes(x = dataFrameMNT[,1], y = 26.20, color = '(5) Alti max.'), size = 0.4, alpha = 0.5, linetype = "dashed")  +
#         geom_line(aes(x = dataFrameMNT[,1], y = 25.21, color = '(4) Alti min.'), size = 0.4, alpha = 0.5, linetype = "dashed")  +
#         geom_line(aes(x = dataFrameMNT[,1], y = 29.79, color = '(5) Alti max.'), size = 0.4, alpha = 0.5, linetype = "dashed")  +
 
         
         geom_line(aes(x = dataFrameMNT[,1], y = dataFrameMNT[,2], color = '(4) Alti min.'), size = 0.4, alpha = 0.5, linetype = "dashed")  +
         geom_line(aes(x = dataFrameMNT[,1], y = dataFrameMNT[,3], color = '(5) Alti max.'), size = 0.4, alpha = 0.5, linetype = "dashed")  +       
...
#         geom_label(label=paste("M13 ",signif(21.47,3),"mNGF",sep=" "), x=140,y=21.47,label.padding = unit(0.30, "lines"), label.size = 0.25, color = "black", fill="white") +
#         geom_label(label=paste("M13 ",signif(26.20,3),"mNGF",sep=" "), x=280,y=26.20,label.padding = unit(0.30, "lines"), label.size = 0.25, color = "black", fill="white") +
#         geom_label(label=paste("M3 ",signif(25.21,3),"mNGF",sep=" "), x=140,y=25.21,label.padding = unit(0.30, "lines"), label.size = 0.25, color = "black", fill="white") +
#         geom_label(label=paste("M3 ",signif(29.79,3),"mNGF",sep=" "), x=280,y=29.79,label.padding = unit(0.30, "lines"), label.size = 0.25, color = "black", fill="white") +
      ...
      print(pl + myGraphOptions + scale_color_manual(values=c("blue1", "green4","red","grey10","grey10","black")) +
